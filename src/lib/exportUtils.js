/**
 * Export utilities for themes
 * Supports PDF, PowerPoint, CSV, and embed code generation
 */

import { getAllNuggets, getComments, getThemeById } from './firestoreUtils';

/**
 * Get all data needed for export
 */
export const getThemeExportData = async (themeId, userId, options = {}) => {
  ...
  let comments = [];
  if (includeComments) {
    comments = await getComments(themeId);
  }
 ...
 * Export to CSV
 */
export const exportToCSV = async (themeId, userId, options = {}) => {
  const { includeComments = false } = options;
  const data = await getThemeExportData(themeId, userId, { includeComments });

  // Create CSV content
  let csvContent = `Theme: ${data.theme.name}\n`;
  csvContent += `Description: ${data.theme.description || ''}\n`;
  csvContent += `Problem Statement: ${data.theme.problemStatement || ''}\n`;
  csvContent += `Created: ${data.theme.createdAt || ''}\n`;
  csvContent += `\n`;
  ...
  link.setAttribute('download', `${data.theme.name.replace(/[^a-z0-9]/gi, '_')}_export.csv`);
 ...
 */
export const exportToPDF = async (themeId, userId, options = {}) => {
  const { includeComments = false } = options;
  const data = await getThemeExportData(themeId, userId, { includeComments });
  ...
  const htmlContent = `
    ...
    <body>
      <h1>${escapeHtml(data.theme.name)}</h1>
      
      <div class="metadata">
        <p><strong>Description:</strong> ${escapeHtml(data.theme.description || 'No description')}</p>
        ${data.theme.problemStatement ? `<p><strong>Problem Statement:</strong> ${escapeHtml(data.theme.problemStatement)}</p>` : ''}
        <p><strong>Created:</strong> ${formatDate(data.theme.createdAt)}</p>
        <p><strong>Total Insights:</strong> ${data.insights.length}</p>
      </div>

      ${data.theme.keyQuestions && data.theme.keyQuestions.length > 0 ? `
        <h2>Key Questions</h2>
        <ul>
          ${data.theme.keyQuestions.map(q => `<li>${escapeHtml(q)}</li>`).join('')}
        </ul>
      ` : ''}
 ...
 */
export const exportToPowerPoint = async (themeId, userId, options = {}) => {
  const { includeComments = false } = options;
  const data = await getThemeExportData(themeId, userId, { includeComments });
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="UTF-8">
      <title>${data.theme.name} - PowerPoint Export</title>
      <style>
        body {
    ...
    <body>
      <div class="slide">
        <h1>${escapeHtml(data.theme.name)}</h1>
        <p>${escapeHtml(data.theme.description || '')}</p>
        <p><strong>Problem Statement:</strong> ${escapeHtml(data.theme.problemStatement || '')}</p>
  ...
  const url = URL.createObjectURL(blob);
  link.setAttribute('href', url);
  link.setAttribute('download', `${data.theme.name.replace(/[^a-z0-9]/gi, '_')}_export.html`);
  link.style.visibility = 'hidden';
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
 ...
 */
export const generateEmbedCode = async (themeId, userId, platform = 'notion') => {
  const data = await getThemeExportData(themeId, userId, { includeComments: false });

  if (platform === 'notion') {
    // Notion embed code (Markdown format)
    let markdown = `# ${data.theme.name}\n\n`;
    markdown += `${data.theme.description || ''}\n\n`;
    
    if (data.theme.problemStatement) {
      markdown += `## Problem Statement\n\n${data.theme.problemStatement}\n\n`;
    }
    
    if (data.theme.keyQuestions && data.theme.keyQuestions.length > 0) {
      markdown += `## Key Questions\n\n`;
      data.theme.keyQuestions.forEach(q => {
        markdown += `- ${q}\n`;
      });
      markdown += `\n`;
    }
    ...
    let html = `<h1>${escapeHtml(data.theme.name)}</h1>`;
    html += `<p>${escapeHtml(data.theme.description || '')}</p>`;

    if (data.theme.problemStatement) {
      html += `<h2>Problem Statement</h2><p>${escapeHtml(data.theme.problemStatement)}</p>`;
    }
